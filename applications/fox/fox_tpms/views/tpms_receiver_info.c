...
static const struct {
    ...
    [TPMSFieldId] = {0, 12, 70, 10},
    [TPMSFieldBattery] = {72, 11, 56, 10},
    [TPMSFieldTemperature] = {2, 39, 42, 23},
    [TPMSFieldPressure] = {44, 39, 46, 23},
};
...
void tpms_view_receiver_info_set_callback(
    ...
    tpms_receiver_info->callback_context = context;
}

TPMSField tpms_view_receiver_info_get_selected_field(TPMSReceiverInfo* tpms_receiver_info) {
    furi_assert(tpms_receiver_info);
    TPMSField sel = TPMSFieldPressure;
    with_view_model(
        tpms_receiver_info->view, TPMSReceiverInfoModel * model, { sel = model->selected; }, false);
    return sel;
}
...
void tpms_view_receiver_info_draw(Canvas* canvas, TPMSReceiverInfoModel* model) {
    ...
    if(model->selected < TPMSFieldCount) {
        ...
        const uint8_t w = tpms_field_boxes[model->selected].w;
        const uint8_t h = tpms_field_boxes[model->selected].h;
        canvas_set_color(canvas, ColorBlack);
        canvas_draw_rframe(canvas, x, y, w, h, 2);
    }
}

static void tpms_view_receiver_info_fire(
    TPMSReceiverInfo* tpms_receiver_info,
    TPMSReceiverInfoAction action) {
    if(tpms_receiver_info->callback) {
        tpms_receiver_info->callback(action, tpms_receiver_info->callback_context);
    }
}
...
bool tpms_view_receiver_info_input(InputEvent* event, void* context) {
        ...
        with_view_model(
            ...
            TPMSReceiverInfoModel * model,
            {
                int next = (int)model->selected +
                           (event->key == InputKeyDown ? 1 : (int)TPMSFieldCount - 1);
                model->selected = (TPMSField)(next % (int)TPMSFieldCount);
            },
            true);
        return true;
    }
    ...
    if(event->key == InputKeyOk && event->type == InputTypeShort) {
        TPMSField sel = TPMSFieldPressure;
        with_view_model(
            tpms_receiver_info->view, TPMSReceiverInfoModel * model, { sel = model->selected; },
            false);
        if(sel == TPMSFieldBattery) {
            tpms_view_receiver_info_fire(tpms_receiver_info, TPMSReceiverInfoActionToggleBattery);
        } else {
            tpms_view_receiver_info_fire(tpms_receiver_info, TPMSReceiverInfoActionEdit);
        }
        return true;
    }
...
static void tpms_view_receiver_info_enter(void* context) {
    ...
    with_view_model(
        tpms_receiver_info->view,
        TPMSReceiverInfoModel * model,
        { model->selected = TPMSFieldPressure; },
        false);
...
TPMSReceiverInfo* tpms_view_receiver_info_alloc() {
    ...
    tpms_receiver_info->callback_context = NULL;
