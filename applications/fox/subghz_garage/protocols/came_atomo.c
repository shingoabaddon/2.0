...
static void subghz_protocol_came_atomo_remote_controller(SubGhzGenericCompat* instance);
...
void* subghz_protocol_encoder_came_atomo_alloc(SubGhzEnvironment* environment) {
    ...
    instance->encoder.size_upload = 900;
...
static void subghz_protocol_encoder_came_atomo_get_upload(
    ...
    if(came_atomo_counter_mode == 0 || bypass) {
        if(subghz_garage_get_rolling_counter_mult() != -0x7FFFFFFF || bypass) {
            bypass = false;
...
SubGhzProtocolStatus subghz_protocol_decoder_came_atomo_serialize(
    ...
    furi_assert(context);
    SubGhzProtocolDecoderCameAtomo* instance = context;
    return subghz_block_generic_serialize((SubGhzBlockGeneric*)&instance->generic, flipper_format, preset);
}
...
SubGhzProtocolStatus
    ...
    status = subghz_block_generic_deserialize((SubGhzBlockGeneric*)&instance->generic, flipper_format);
