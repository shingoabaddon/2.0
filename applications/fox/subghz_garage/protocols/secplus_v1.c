...
#define SECPLUS_V1_BIT_ERR -1
#define SECPLUS_V1_BIT_0   0
#define SECPLUS_V1_BIT_1   1
#define SECPLUS_V1_BIT_2   2
...
static bool subghz_protocol_secplus_v1_encode(SubGhzProtocolEncoderSecPlus_v1* instance) {
    ...
    if(subghz_garage_get_rolling_counter_mult() != -0x7FFFFFFF) {
        if(!subghz_garage_counter_override_get(&rolling)) {
            if((rolling + subghz_garage_get_rolling_counter_mult()) > 0xFFFFFFFF) {
                rolling = 0xE6000000;
            } else {
                rolling += subghz_garage_get_rolling_counter_mult();
            }
        }
