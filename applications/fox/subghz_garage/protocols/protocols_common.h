 ...
 * gen_data_protocol path) guard their encoder-side code with this so the
 * RX-only group plugins don't carry TX signal-generation code they never
 * use - see subghz_txrx_ensure_tx_protocol_plugin() in helpers/subghz_txrx.c
 * for where the per-protocol TX plugins that DO need it are loaded. */
#if !defined(SUBGHZ_GARAGE_RX_ONLY)
#define SUBGHZ_GARAGE_WITH_ENCODER 1
#else
#define SUBGHZ_GARAGE_WITH_ENCODER 0
#endif
...
#undef PP_SHARED_UPLOAD_CAPACITY
#define PP_SHARED_UPLOAD_CAPACITY 32768U
