...
static void variable_item_list_draw_callback(Canvas* canvas, void* _model) {
    ...
    for(VariableItemArray_it(it, model->items); !VariableItemArray_end_p(it);
        ...
        uint8_t items_on_screen = items_on_screen_base;
        uint8_t y_offset = 0;
        ...
        if(item_position < items_on_screen) {
            ...
            if(position == model->position) {
                ...
                if(fox) {
                    canvas_draw_rbox(canvas, 0, item_y + 1, item_width, item_height - 2, 4);
                } else {
                ...
                canvas_set_color(canvas, ColorBlack);
                if(fox) {
                    canvas_draw_rframe(canvas, 0, item_y + 1, item_width, item_height - 2, 4);
                }
            }

            bool has_value = !(item->current_value_index == 0 &&
                               furi_string_empty(item->current_value_text));
            ...
            if(fox) {
                if(!has_value) {
                    ...
                    if(item->locked) {
                        canvas_draw_icon(
                            canvas, item_width - 12, item_mid_y - 4, &I_Lock_7x8);
                    } else {
                        const char* val_str =
                        ...
                        if(item->current_value_index > 0) {
                            canvas_draw_str_aligned(
                                canvas, 67, item_mid_y, AlignLeft, AlignCenter, "<");
                        }
                        canvas_draw_str_aligned(
                            canvas, 96, item_mid_y, AlignCenter, AlignCenter, val_str);
                        if(item->current_value_index < (item->values_count - 1)) {
                            canvas_draw_str_aligned(
                                canvas, item_width - 2, item_mid_y, AlignRight, AlignCenter, ">");
                        }
                    }
                }
            } else {
                ...
                if(!has_value) {
                    ...
                    elements_scrollable_text_line_str(
                        canvas, 6, item_text_y, temp_w_default,
                        item->label, scroll_counter, false, false);
                }
                ...
                if(item->locked) {
                    ...
                    if(has_value) {
                        ...
                        elements_scrollable_text_line_str(
                            canvas, (115 + temp_x_default) / 2 + 1, item_text_y, 37,
                            furi_string_get_cstr(item->current_value_text),
