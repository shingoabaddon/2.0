...
#include <furi.h>
#include "canvas.h"
#include "icon_animation.h"   // needed for IconAnimation* in elements_fox_horizontal_menu_item
...
/** Draw a single Fox Theme grid menu cell.
 *
 * @param canvas    Canvas instance
 * @param x         cell left edge
 * @param y         cell top edge
 * @param width     cell width  (typically 40)
 * @param height    cell height (typically 30)
 * @param label     item label string
 * @param icon      Icon instance (14×14 recommended), may be NULL
 * @param selected  true to draw filled/inverted (selected) state
 */
void elements_fox_horizontal_menu_item(
    ...
    size_t height,
    const char* label,
    IconAnimation* icon,   // was const Icon* — now takes live animation for per-frame drawing
    bool selected);
