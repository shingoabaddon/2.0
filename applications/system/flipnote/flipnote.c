...
int32_t flipnote_app(void* p) {
    ...
    /* Fox File Browser's "Run in app" / "Open With" launches us with the
       tapped file's full path as the argument (same convention mp3_player's
       app_main uses) - open it immediately instead of landing on a blank
       new file. Same sequence as the manual File > Open menu action. */
    if(args && args[0]) {
        strncpy(m->filename, args, sizeof(m->filename) - 1);
