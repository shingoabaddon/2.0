...
static void write_problems_view_draw_callback(Canvas* canvas, void* _model) {
    ...
    canvas_draw_str_aligned(canvas, 96, 0, AlignCenter, AlignTop, furi_string_get_cstr(header));

    // Warning message
    canvas_set_font(canvas, FontSecondary);
    elements_text_box(
        canvas, 66, 13, 60, 42, AlignLeft, AlignTop, furi_string_get_cstr(model->content), false);
