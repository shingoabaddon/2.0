...
// Portable block-layout and validation helpers used by bambu.c.
//
...
//
// Include after mf_classic.h (MfClassicData/MfClassicType1k already defined).
...
// buffer starts zeroed and nothing overwrites it, so the parser would
// otherwise render a 0.00mm nozzle, a 0.00mm spool width and a blank
// production date as if they were genuine. Requiring all six blocks
// simultaneously - including the two the plugin never renders - makes this a
// statement about the sectors being absent rather than about any single
// field being an implausible value.
...
static inline bool bambu_tag_is_valid(const MfClassicData* data) {
    ...
    // dumps saved without "Data format version: 2", so a pre-v2 dump that is
    // itself partial has no read-state left to consult.
