...
void lfrfid_scene_t5577mw_write_first_key_on_enter(void* context) {
    LfRfid* app = context;
    ...
    t5577mw_add_em41xx_data(&data_to_write, key, 1);
    t5577mw_set_em41xx_config(&data_to_write, 1);
...
bool lfrfid_scene_t5577mw_write_first_key_on_event(void* context, SceneManagerEvent event) {
    LfRfid* app = context;
    Popup* popup = app->popup;
    bool consumed = false;
...
void lfrfid_scene_t5577mw_write_first_key_on_exit(void* context) {
    LfRfid* app = context;
